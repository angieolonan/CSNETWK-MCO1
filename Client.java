...
public class Client {
    ...
    //                                                                       0         1          2           3        4       5      6      7
    //ArrayList<String> keywords = new ArrayList<String>(Arrays.asList("/join", "/leave", "/register", "/store", "/dir", "/get", "/?", "/end"));
    String keywords[] = {"/join", "/leave", "/register", "/store", "/dir", "/get", "/?", "/end"};
    ...
    public void UI () {
        ...
        while (true) {
            ...
            String comSplit[] = command.split(" ", -2);
            ...
            if (comSplit[0].equals(keywords[0])) {
                if (comSplit.length == 3) {
                    ...
                    int portNum = Integer.parseInt(comSplit[2]);
            ...
            if (!check) {
                ...
                // /leave
                if (comSplit[0].equals(this.keywords[1])) {
                    if (comSplit.length == 1) {
                        this.disConnect(check);
                    }
                    else {
                ...
                // /register
                if (comSplit[0].equals(this.keywords[2])) {
                ...
                // /store
                if (comSplit[0].equals(this.keywords[3])) {
                    if (comSplit.length == 2) {
                ...
                // /dir
                if (comSplit[0].equals(this.keywords[4])) {
                    if (comSplit.length == 1) {
                        this.listFiles();
                    } else {
                        System.out.println("\nERROR: Command does not have parameters");
                ...
                // /get
                if (comSplit[0].equals(this.keywords[5])) {
                    if (comSplit.length == 2) {
            ...
            // /?
            if (comSplit[0].equals(this.keywords[6])) {
                if (comSplit.length == 1) {
                    this.dispCommands();
                }
                else {
            ...
            if (comSplit[0].equals(this.keywords[7])) {
                if (comSplit.length != 1) {
                    System.out.println ("\nERROR: Command does not have parameters");
                }
                else if (check && !(disConnect(check))) {
    ...
    public boolean createConnect (String address, int portNum, Socket clientSocket) {
        ...
        try {
            ...
            SocketAddress socketAddress = new InetSocketAddress(ipAddress, portNum);

            clientSocket.connect(socketAddress, portNum);
    ...
    public boolean disConnect (boolean check) {
        boolean result = false;
